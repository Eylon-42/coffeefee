 ...
 * Use FeedRepository from com.eaor.coffeefee.repository package instead,
 * which utilizes Room for better database management.
 */
@Deprecated("Use FeedRepository instead which utilizes Room for better database management")
class PostDatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    // Method to insert a batch of posts in a transaction
    fun insertPosts(posts: List<FeedItem>) {
        ...
        try {
            ...
            Log.e("PostDatabaseHelper", "Error inserting posts: ${e.message}", e)
        } finally {
            if (db.inTransaction()) {
                db.endTransaction()
            }
    ...
    // Method to insert a single post
    fun insertPost(post: FeedItem) {
        ...
        try {
            ...
            Log.e("PostDatabaseHelper", "Error inserting post: ${e.message}", e)
        }
    }
    ...
    fun getAllPosts(): List<FeedItem> {
        ...
        try {
            ...
            Log.e("PostDatabaseHelper", "Error reading posts: ${e.message}", e)
        } finally {
            cursor.close()
        }
        return posts
    ...
    fun clearPosts() {
        ...
        try {
            ...
            Log.e("PostDatabaseHelper", "Error clearing posts: ${e.message}", e)
        }
    }
} 