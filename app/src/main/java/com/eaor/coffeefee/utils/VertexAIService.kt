...
class VertexAIService private constructor() {
    ...
    private fun buildPrompt(
        ...
        return """
            ...
            Keep it conversational and authentic. The description should be maximum 15 words,
    ...
    /**
     * Generate tags based on a post description
     * @param description The post description
     * @return Result containing either success with a list of tags or failure with exception
     */
    suspend fun generateTags(description: String): Result<List<String>> = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "Generating tags for description: $description")
            
            // Create a generative model
            val generativeModel = vertexAI.generativeModel(MODEL_NAME)
            
            // Build the prompt for tag generation
            val prompt = "Generate a list of general tags (maximum 5) that can be understood from this post: '$description'. Return only the tags, separated by commas."
            ...
            if (response.text.isNullOrEmpty()) {
                Log.e(TAG, "Empty response from VertexAI for tag generation")
                Result.failure(Exception("Empty response from AI"))
            } else {
                Log.d(TAG, "Successfully generated tags: ${response.text}")
                
                // Parse and clean up tags
                val tagList = response.text!!.split(",").map { it.trim() }.filter { it.isNotEmpty() }
                Result.success(tagList)
            }
        } catch (e: Exception) {
            Log.e(TAG, "Exception during tag generation", e)
            Result.failure(e)
        }
    }
     ...
     * @param favoriteDrink The user's favorite coffee drink
     * @param dietaryNeeds The user's dietary needs
     * @param atmosphere The user's preferred atmosphere
     * @param location The user's preferred location
     * @return Result containing either success with a list of tags or failure with exception
     */
    suspend fun generateTagsFromPreferences(
        ...
        try {
            Log.d(TAG, "Generating tags from user preferences")
            
            // Build the prompt for tag generation using existing method
            val prompt = buildTagGenerationPrompt(favoriteDrink, dietaryNeeds, atmosphere, location)
            ...
            if (response.text.isNullOrEmpty()) {
                Log.e(TAG, "Empty response from VertexAI for preference tag generation")
                Result.failure(Exception("Empty response from AI"))
            } else {
                Log.d(TAG, "Successfully generated preference tags: ${response.text}")
            ...
            Log.e(TAG, "Exception during preference tag generation", e)
            Result.failure(e)
        }
    }
